...
const FelteAsyncDeco = defineAsyncConstraint<string, undefined>({
    name: 'solidFelteAsyncDeco',
    validate: async (value) => value !== 'taken',
    message: 'Already taken'
});

describe('@hermiforge-decorix/solid-felte', () => {
    ...
    it('creates a Felte config from builder metadata and can attach Zod validation', () => {
        registerZodValidator({name: 'zod-solid-felte-builder'});
    ...
    it('resolves async constraints through validateAsync', async () => {
        createAsyncConstraint<unknown, undefined>({
            name: 'solidFelteAsyncAvailable',
            validate: async (value) => value !== 'taken',
            message: 'Already taken'
        });
        const metadata = model('FelteAsyncDto', {
            username: stringField().required().constraint('solidFelteAsyncAvailable')
        });
    ...
    it('wires into a real @felte/solid createForm instance without throwing', () => {
        // A full assertion on `errors()` after `setFields`/`validate()` proved
        // too brittle outside a mounted Solid component (reactive updates and
        // the exact `setFields` signature depend on the rendering owner) —
        // the same class of constraint documented for angular-signal's
        // `form()`. This still proves `createForm` accepts our config as-is
        // (real devDependency, real call) instead of only asserting on the
        // config's own shape.
        createRoot((dispose) => {
            const metadata = model('FelteRealDto', {
                name: stringField().required().minLength(2, 'Name too short')
            });
            const config = toFelteForm(metadata);
